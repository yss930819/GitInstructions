...
- [4. Git的协作之旅](#4-git的协作之旅)
    - [4.1. 本地已有版本库](#41-本地已有版本库)
    - [4.2. 服务端新建版本库](#42-服务端新建版本库)
    - [4.3. 迁移其他服务器的版本库](#43-迁移其他服务器的版本库)
...
本例还以`TestGitCC`项目为例说明，在项目中增加一个**add.c**文件，其中
...
在使用Git之后查看历史版本的文件将变得跟加简单和轻松。这里以`gitea`的项目为例说明

#### 2.6.1. 查看一个文件或文件夹的历史版本
*这里选用`gitea`中的doc文件夹为例*  

1. 找到想查看的文件或文件夹，选中并在其上右键，点击**TortoiseGit**->**显示日志**。  
![git_use_local100](./img/git_use_local100.png)

1. 弹出的对话框中上半部分显示了该文件或文件夹的所有**提交信息**，下版本分显示了该次提交**修改过的文件**，对于选中的为文件或文件夹会高亮为**蓝色**。
...
1. 通过[http://127.0.0.1:3000/](http://127.0.0.1:3000/)访问Gitea。

1. 首次访问需要对Gitea进行配置，配置说明如下图所示，
...
1. 再次访问[http://127.0.0.1:3000/](http://127.0.0.1:3000/)。  
